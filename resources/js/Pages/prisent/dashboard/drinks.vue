<template>
    ...
    <div class="main-board-mealxpress">
      ...
      <AddDrinks :addVisible="addVisible" @close="CloseCenterModel" @submitmodal="submitmodal"  :isOptionLoading="isOptionLoading" />
      <EditVendors/>
        <NavbarComponent />
                    ...
                    <div class="d-flex justify-content-between px-6">
                        ...
                        <div class="d-flex gap-4">
                            ...
                            <div class="meal-form-button">
                                <button @click="ShowCenterModel(item)" class="togglebutton btn bg-button-submit py-3 text-sm">Add Drinks</button>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                    <div class="table-responsive text-nowrap">
                      ...
                      <tbody>
                          ...
                          <tr v-for="(item, index) in paginatedData" :key="item.id">
                            ...
                            <td>  <span class="fw-medium">{{item.drinkname}}</span></td>
                            <td>  <span class="fw-medium">₦ {{formattedPrice(item.drinkprice)}}</span></td>
                             <td><img :src="item.drinkimage" class="img-rounded" style="width: 60px; height:60px;"></td>
                            <td>
  ...
  import AddDrinks from './../../components/modals/adminmodals/addDrinks.vue';
  import DeleteModel from './../../components/modals/adminmodals/deletedrinks.vue';
  ...
  await delay(5000);
  try {
    const response = await axios.post('/prisent/dashboard/drinksdelete', payload, {
      headers: {
    ...
    const submitmodal = async (data) => {
      ...
      await delay(3000);
      const response = await axios.post('/prisent/dashboard/add_drinks', data,{
        headers: {
            'X-CSRF-TOKEN': csrfToken,
          },
      });
