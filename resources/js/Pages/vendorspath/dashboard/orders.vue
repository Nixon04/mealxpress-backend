    ...
    <div class="main-board-mealxpress">
      ...
      <Tabs value="0" class="tablets custom-tabs px-3 m-2">
        <TabList class="bg-meal-tablist">
            <Tab value="0" @click="selectType(0)">All</Tab>
            <Tab value="1" @click="selectType(1)">Processing</Tab>
            <Tab value="2" @click="selectType(2)">Completed</Tab>
            <Tab value="3" @click="selectType(3)">Returns</Tab>
        </TabList>
            ...
            <div class="card-datatable text-nowrap table-responsive">
              <table class="dt-fixedcolumns table  ">
                ...
                <tbody>
                  ...
                  <tr v-for="(item, index) in paginatedData" :key="index.id" @click="ViewAllTogether(item)" class="cursor-pointer" >
                      <td >{{ item.username }}</td>
  ...
  export default {
    ...
    const formatCurrency = (amount, locale = "en-NG", currency = 'NGN') => {
      return new Intl.NumberFormat(locale,{
        style:'currency',
        currency: currency,
      }).format(amount); // Simple number formatting
    };
      ...
      var itemselected = ref(data.value);

      const selectType = (item) => {
      if (item == 0 || item == null) {
        itemselected.value = data.value;
      } else if (item == 1) {
        itemselected.value = pendingtable.value;
      } else if (item == 2) {
        itemselected.value = deliveredtable.value;
      } else if (item == 3) {
        itemselected.value = returnstable.value;
      } else {
        itemselected.value = data.value;
      }
      currentPage.value = 1;
  ...
  const filteredData = computed(() => {
    ...
    if (!searchQuery.value) return itemselected.value;

    return itemselected.value.filter((item) =>
